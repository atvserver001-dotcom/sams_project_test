...
export default function PreviewControls() {
  ...
  async function reset() {
    if (!await confirmAction('이 브라우저의 관리자 샘플 변경 내용과 측정 세션을 초기화할까요?')) return
    setBusy(true)
    try {
  ...
  return <main className="mx-auto max-w-[640px] px-7 py-12">
    <h1 className="mb-8 text-2xl font-extrabold">관리자 샘플 미리보기</h1>
    <dl className="mb-8 divide-y border-y border-border">
      <div className="flex justify-between gap-5 py-4"><dt>데이터</dt><dd>가상 학교 · 계정 · 기기</dd></div>
    ...
    <div className="flex flex-wrap gap-3">
      <Button asChild variant="outline"><Link href="/admin"><ArrowLeft />관리자 화면</Link></Button>
